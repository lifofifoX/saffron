...
As of the review date, `npm audit --omit=dev` reports 15 low-severity dependency entries and no
moderate, high, or critical entries. Exact npm overrides keep Trezor on `protobufjs` 7.6.5 and
deduplicate `sats-connect` onto Axios 1.19.0 and Valibot 1.4.2. These are compatibility bridges until
the parent packages publish those fixed dependencies; keep the physical regression requirement
above when changing them.

`npm audit signatures` verifies registry signatures for 501 installed packages and attestations for 102. Signatures establish package provenance, not runtime safety. Repository `.npmrc` disables all
